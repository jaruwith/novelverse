...
export function DesktopNavigation(){return <nav className={styles.nav}><Link href="/">หน้าแรก</Link><Link href="/creator/stories">พื้นที่ครีเอเตอร์</Link><Link href="/login">เข้าสู่ระบบ</Link></nav>}
export function MobileNavigation({close}:{close:()=>void}){return <div className={styles.drawer}><button className="textButton" onClick={close}>✕ ปิดเมนู</button><Link href="/">หน้าแรก</Link><Link href="/creator/stories">พื้นที่ครีเอเตอร์</Link><Link href="/login">เข้าสู่ระบบ</Link></div>}
export function GlobalHeader(){const [drawer,setDrawer]=useState(false);return <header className={styles.header}><div className={styles.headerRow}><Link className={styles.brand} href="/">NovelVerse<small>J007LNWZA</small></Link><DesktopNavigation/><GlobalSearch/><button className={styles.mobileButton} onClick={()=>setDrawer(true)}>☰</button>{drawer&&<MobileNavigation close={()=>setDrawer(false)}/>}</div></header>}
